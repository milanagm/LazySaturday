...
1. **Start the backend stack (FastAPI + Postgres + Redis + n8n)**
   ```bash
   make backend
   ```
...
For quick iteration you can still run the FastAPI app locally:
...
Make sure Postgres, Redis, and n8n are available (e.g., via `docker compose up postgres redis n8n`).
